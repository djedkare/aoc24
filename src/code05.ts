...
export const prepareInput = (
    ...
    const arr_ = s.split('\n');
    const arr =
        arr_[arr_.length - 1] === '' ? arr_.slice(0, arr_.length - 1) : arr_;
    const splitPoint = arr.indexOf('');
    const constraints: [number, number][] = arr
        .slice(0, splitPoint)
        .map(
            (row) => row.split('|').map((n) => parseInt(n)) as [number, number]
        );
    const updates: number[][] = arr
        .slice(splitPoint + 1)
...
export const makePredecessorMap_ = (
    constraints: [number, number][]
...
export const makePredecessorMap = (obj: {
    constraints: [number, number][];
    updates: number[][];
    pages: Set<number>;
}): Map<number, Set<number>> => {
    ...
    obj.pages.forEach((page) =>
        !map.has(page) ? map.set(page, new Set([])) : false
    );
    return map;
};

export const isPredecessorOf = (
...
export const validateUpdate = (
    ...
    return update.every((page, i) =>
        update
            .slice(i + 1)
            .every((page_) => !isPredecessorOf(predecessors, page_, page))
    );
};

export const middleElement = <T>(l: T[]): T | typeof NaN =>
    l.length % 2 === 1 ? l[Math.floor(l.length / 2)] : NaN;
...
export const solve = (str: string): number => {
    ...
    obj.updates.forEach((upd) =>
        validateUpdate(m, upd) ? (n += middleElement(upd)) : false
    );
    return n;
};
...
export const transitivePredecessors_ = (
    predecessors: Map<number, Set<number>>,
    succ: number,
    set: Set<number>
): void => {
...
export const transitivePredecessors = (
    predecessors: Map<number, Set<number>>,
    succ: number
): Set<number> => {
...
export const transitivePredecessorMap = (
    predecessors: Map<number, Set<number>>
): Map<number, Set<number>> => {
...
export const topologicalSort = (
    transPredMap: Map<number, Set<number>>,
    list_: number[]
): number[] => {
    const list = list_.slice();
    let isPredOf = (a: number, b: number) =>
        isPredecessorOf(transPredMap, a, b);
...
export const bubbleSort = (
    ...
    while (sortedArr.length < inputArr.length) {
        ...
        unsortedElems.forEach((n) => {
            if (isEmptySet(intersection(transPredMap.get(n)!, unsortedElems))) {
                addedNext.push(n);
            }
        });
