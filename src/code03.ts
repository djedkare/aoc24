...
export const getMulResult = (str: string): number => {
    ...
    return (
        parseInt(str.slice(n0_start, n0_end)) *
        parseInt(str.slice(n1_start, n1_end))
    );
};

export const solution0: number = arr.reduce(
    (acc, cur) => acc + getMulResult(cur),
    0
);
...
export const parseDo = (str: string): string | false =>
    parseString('do()')(str);

export const parseDont = (str: string): string | false =>
    parseString("don't()")(str);
