...
export const valid = ([y, x]: [number, number]) =>
    y < arr.length && x < arr[y].length;
...
export const arcs: string[] = ([] as string[])
    .concat(
        ...
        U.range(140)
            .slice(1)
            .map((x) => makeString([0, x], [1, 1])),
        // diagonal down left:
        U.range(140)
            .slice(1)
            .map((y) => makeString([y, 139], [1, -1])),
...
export const solution1 = U.range(139)
    .slice(1)
    .flatMap((y) =>
        U.range(139)
            .slice(1)
    ...
    .filter(([y, x]) => checkMas(y, x)).length;
